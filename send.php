<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        $pesan = $_POST['message'];
        $_SESSION['message'] = $pesan; // Store message in session variable

        // Redirect with message_sent parameter
        header("Location: profile.php?message_sent=true");
        exit();
    }
    ...
    if (isset($_SESSION["success_message"])) {
        ...
        unset($_SESSION["success_message"]); // Hapus pesan dari sesi agar tidak ditampilkan lagi
    }
    
    if (isset($_SESSION["error_message"])) {
        echo '<div class="alert alert-danger">' . $_SESSION["error_message"] . '</div>';
        unset($_SESSION["error_message"]); // Hapus pesan dari sesi agar tidak ditampilkan lagi
